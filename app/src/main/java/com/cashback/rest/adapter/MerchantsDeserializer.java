    ...
    public List<Merchant> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws ErrorRestException, WarningRestException, JsonParseException {
        ...
        if (status.equals(ServiceGenerator.REQUEST_STATUS_OK)) {
            ...
            if (total > 0) {
                ...
                JsonElement merchants = dataObj.get("merchants");
                if (merchants.isJsonArray()) {
                    for (JsonElement element : merchants.getAsJsonArray()) {
                        resultList.add(gson.fromJson(element, Merchant.class));
                    }
                }
            }
            return resultList;
